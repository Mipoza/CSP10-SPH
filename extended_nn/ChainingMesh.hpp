...
struct ChainingMeshHelper{
  ...
  void add_neighbor_cells(
      ...
      const IDX_TYPE r = 1) const {

#ifdef _DEBUG
    if(NN[DIR]/2 < r){
      std::cerr << "Why is this radius so big?"
                << std::endl;
      abort();
    }
#endif
    ...
    if constexpr (DIR == -1){
      ...
      for(i = 0; (temp[DIR] + 1 < NN[DIR]) && (i < r); ++i){
        ++temp[DIR];
        add_neighbor_cells<DIR - 1>(n_list, temp, r);
      }
      ...
      // 0, add self/center
      add_neighbor_cells<DIR - 1>(n_list, current, r);
    }
  }

  // Get a list of indices of cells neighboring ``current''
  inline std::vector<IDX_TYPE> 
  get_cell_neighbor_idx(std::array<IDX_TYPE, DIM>& current, const IDX_TYPE r = 1) const {
    std::vector<IDX_TYPE> res;
    // Reserve some memory in anticipation
    res.reserve(NNCells);
    add_neighbor_cells<DIM - 1>(res, current, r); 
    return res;
  }
